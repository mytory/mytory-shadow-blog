...
\***************************************************************************/
$BLOGGER_APPID  = "656278";	// Blog ID
$BLOGGER_SERVER = "mytory.tistory.com"; // Blog API 셋팅 주소
$BLOGGER_PATH   = "/api";	// Blog API 셋팅 주소
...
function blogger_newPost($blogid, $username, $password, $cate, $tags, $content, $subject, $accept_comments, $accept_trackback, $publish, $reg_date) {
	global $BLOGGER_APPID;
	...
	// Construct the actual request message
	$newPostRequest = new xmlrpcmsg("metaWeblog.newPost", 
		array(
			...
			new xmlrpcval(
			array(
				"categories" => new xmlrpcval(array(new xmlrpcval(iconv("EUC-KR","UTF-8",$cate), "string")), "array"),
				"mt_keywords"=>new xmlrpcval(iconv("EUC-KR","UTF-8",$tags), "string"),  // 태그 , 으로 구분
				"description"=>new xmlrpcval(iconv("EUC-KR","UTF-8",$content), "string"),  //내용
				"title"=>new xmlrpcval(iconv("EUC-KR","UTF-8",$subject), "string"),  //제목
				"mt_allow_comments"=>new xmlrpcval($accept_comments, "string"), //댓글허용 =1
				"mt_allow_pings"=>new xmlrpcval($accept_trackback, "string"),  //트랙백허용 =1
				"dateCreated"=>new xmlrpcval($reg_date, "string")  //날짜
			), "struct"),
			new xmlrpcval($publish, "boolean") ) // 공개여부 공개 =1 비공개=0
	);
